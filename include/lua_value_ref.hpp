...
class lua_value_ref
{
  ...
  public:
     ...
     * m_name + "_" + random number generated by stl's default random engine and uniform int distribution
     * \note This done in order to eliminate the following scenario.\n
     * Let's say the registered function 'something' just return the argument from the input, if 
     * we use the same name the lua_value_ref object 'a' and 'b' will both have a value of 2 at the end
