...
T lua_stack::top_element(bool popTopElement) const
{
    if (popTopElement) {
        ...
        // //Change the value from m_state to see if the value off d
        // state changes push(s+1); lua_setglobal(m_state,
        // "integer_var"); lua_getglobal(d, "integer_var");
        ...
        //     //Try pushing element to the stack,segmentation fault if
        //     cannot insert more elements for(int i=0;i!=12000;++i)
        //     {
        ...
        //     //The __index of this table points to the old
        //     LUA_GLOBALSINDEX,
        ...
        //     lua_pushvalue(thread2, LUA_GLOBALSINDEX); //original
        //     globals lua_settable(thread2, -3);
        //     lua_setmetatable(thread2, -2);
        //     //lua_replace(thread2, LUA_GLOBALSINDEX); //replace
        //     newState's globals
        ...
        //     auto s=LUA_GLOBALSINDEX;//This is deprecated from Lua5.2
        //     onwards auto dummyMainState=lua_tointeger(m_state,top);
        ...
        //     auto
        //     dummysecondMainState=lua_tointeger(secondMainState,top);
        ...
        auto dep1state =
            LuaBz::detail::lua_state_factory::create_state(file_name1);
        lua_getglobal(dep1state, "dep1_integer");
        auto dep1 = lua_tointeger(dep1state, top);

        auto dep2state =
            LuaBz::detail::lua_state_factory::create_state(file_name2);
...
template <class T>
typename std::enable_if<std::is_convertible<T, lua_table>::value>::type
lua_stack::push(T val) const
{
...
{
    using DispatchingTypes = std::tuple<detail::ClassType, detail::IntegralType,
                                        detail::FloatingPointType>;
    using Conditions = Utils::metafunction_list<std::is_class, std::is_integral,
                                                std::is_floating_point>;
    return get_impl<T>(
        index, typename Utils::static_type_if<T, Conditions,
                                              DispatchingTypes>::type{});
}
template <>
...
{
    static_assert(std::is_constructible<T, lua_table>::value,
                  "Type cannot be constructed from a lua_table");
