...
std::tuple<R...> lua_script::call(const lua_function<R...> &f,
                                  Args &&... args) const
{
...
std::pair<First, Second> lua_script::call(const lua_function<First, Second> &f,
                                          Args &&... args) const
{
    load_function(f.name());
    const int inputCount = set_parameters(std::forward<Args>(args)...);
...
std::pair<First, Second> lua_script::call(
    const lua_function<First, Second> &f) const
{
    load_function(f.name());
...
template <class R, class... Args>
void lua_script::register_function(const std::string &name,
...
{
    // WARNING: Not thread safe
    m_registeredFunctions.emplace_back([this,
                                        user_f](lua_State *) mutable -> int {
...
int lua_script::call_registered_function(T &user_f,
                                         ...
                                         std::tuple<Args...> &)
{
    ...
    auto result = user_f(
        m_stack.get_element<Args>((index_generator.get_index() + 1) * -1)...);
