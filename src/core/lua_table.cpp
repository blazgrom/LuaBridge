...
typename std::vector<lua_value>::reference lua_table::at(
    typename std::vector<lua_value>::size_type pos)
{
    return m_values.at(pos);
}
typename std::vector<lua_value>::const_reference lua_table::at(
    typename std::vector<lua_value>::size_type pos) const
{
    return m_values.at(pos);
}
typename std::vector<lua_value>::reference lua_table::operator[](
    typename std::vector<lua_value>::size_type pos)
{
    return m_values[pos];
}
typename std::vector<lua_value>::const_reference lua_table::operator[](
    typename std::vector<lua_value>::size_type pos) const
{
    return m_values[pos];
}
...
typename std::vector<lua_value>::const_iterator lua_table::begin() const
    noexcept
{
    return m_values.begin();
}
typename std::vector<lua_value>::const_iterator lua_table::cbegin() const
    noexcept
{
...
typename std::vector<lua_value>::const_reverse_iterator lua_table::rbegin()
    const noexcept
{
    return m_values.rbegin();
}
typename std::vector<lua_value>::const_reverse_iterator lua_table::crbegin()
    const noexcept
{
...
typename std::vector<lua_value>::const_reverse_iterator lua_table::rend() const
    noexcept
{
    return m_values.rend();
}
typename std::vector<lua_value>::const_reverse_iterator lua_table::crend() const
    noexcept
{
...
bool operator<(const lua_value &lhs, const lua_value &rhs)
{
    ...
    // lua_typesX < Nil -> false, because it's not possible to compare types
    // with nil in Lua  Nil < lua_typesX -> true, see lua_typesX < Nil. True
    // because we have inverted the position of Nil  lua_typesX < lua_typesX
    // -> compare type if possible, if not possible returns false
...
{
    // Note:
    // Two lua_values compare equal if they have the save type and the value
    ...
    if (sameType) {
        switch (lhs.type()) {
            ...
            case lua_types::Number: {
                ...
                auto absoluteDifference = std::fabs(lhs.get<lua_t::number>() -
                                                    rhs.get<lua_t::number>());
            ...
            case lua_types::Table: {
                if (lhs.get<lua_table>().size() !=
                    rhs.get<lua_table>().size()) {
                    return false;
                }
...
lua_value::lua_value() : m_type{lua_types::Nil}, m_name{""}, m_nil{lua_t::nil{}}
{
}
lua_value::lua_value(const std::string &name, lua_t::nil data)
  : m_type{lua_types::Nil}, m_name{name}, m_nil{data}
{
}
lua_value::lua_value(const std::string &name, lua_t::integer data)
  : m_type{lua_types::Integer}, m_name{name}, m_integer{data}
{
}
lua_value::lua_value(const std::string &name, lua_t::number data)
  : m_type{lua_types::Number}, m_name{name}, m_number{data}
{
}
lua_value::lua_value(const std::string &name, lua_t::boolean data)
  : m_type{lua_types::Boolean}, m_name{name}, m_bool{data}
{
}
...
lua_value::lua_value(const std::string &name, const lua_t::string &data)
  : m_type{lua_types::String}, m_name{name}, m_string{data}
{
}
lua_value::lua_value(const std::string &name, const lua_table &data)
  : m_type{lua_types::Table}, m_name{name}, m_table{data}
{
}
lua_value::lua_value(const lua_value &rhs)
  : m_type{rhs.m_type}, m_name{rhs.m_name}
{
    init(rhs);
}
lua_value::lua_value(lua_value &&rhs) : m_type{rhs.m_type}, m_name{rhs.m_name}
{
    init(rhs);
}
