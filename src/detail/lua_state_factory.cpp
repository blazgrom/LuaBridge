    ...
    namespace detail
    {
        ...
        */
        lua_State* lua_state_factory::create_state(const std::string& file_name)
        {
            ...
            bool already_opened=active_lua_states.find(registry_key) !=active_lua_states.end();
            if(already_opened){
                lua_rawgeti(master_state,LUA_REGISTRYINDEX,active_lua_states[registry_key]);
